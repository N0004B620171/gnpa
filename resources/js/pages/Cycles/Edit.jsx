...
export default function Edit({ cycle, langues, systemes }) {
    const { errors } = usePage().props;
    const [selectedLangues, setSelectedLangues] = useState(cycle.langues?.map(l => l.id) || []);
    const [selectedSysteme, setSelectedSysteme] = useState(cycle.systeme);

    const { data, setData, put, processing } = useForm({
        nom: cycle.nom,
        systeme: cycle.systeme,
        nombre_trimestres: cycle.nombre_trimestres,
        bareme: cycle.bareme,
        langues: cycle.langues?.map(l => l.id) || []
    });
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-cyan-100 mt-2 text-lg">
                                Mettez à jour les informations de {cycle.nom}
                            </p>
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-2xl p-6 border border-cyan-200">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-cyan-100">
                                <div>
                                    <div className="font-medium text-cyan-900">{data.nom}</div>
